...
namespace EuroJudoWebContestSheets.Generators
{
    ...
    {
        public static string GenerateSVGImage(Category category)
        {
            string SVG = "";

            SVG = SVG + "<?xml version=\"1.0\" encoding=\"UTF-8\"?>";
            if(category.SheetSize < 5)
            {
                SVG = SVG + "<svg viewBox=\"300 100 950 400\" xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\">";
            }
            else if(category.SheetSize < 9)
            {
