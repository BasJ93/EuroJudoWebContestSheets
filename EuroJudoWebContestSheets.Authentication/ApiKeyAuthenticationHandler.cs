...
using EuroJudoWebContestSheets.Authentication.JSON;
    ...
    public class ApiKeyAuthenticationHandler : AuthenticationHandler<ApiKeyAuthenticationOptions>
{
    ...
    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        if (!Guid.TryParse(providedApiKey, out Guid guidKey))
        {
            return AuthenticateResult.NoResult();
        }
        
        var existingApiKey = await _apiKeyRepository.ByKey(guidKey);
        ...
        if (existingApiKey != null)
        {
            ...
            claims.AddRange(existingApiKey.Roles.Select(role => new Claim(ClaimTypes.Role, role.Name)));
